/*
    ...
    Terse mode output, getopt() command line processing, optional stdin input, and HTML documentation added in October 1998.
    Documentation for the -t (terse output) option added in July 2006.
    Replaced table look-up for chi square to probability conversion with algorithmic computation in January 2008.

    For additional information and the latest version, see https://www.fourmilab.ch/random/
...
/*  Main program, change to interface  */
// int main(int argc, char *argv[])

/* ---------------+-----------+-------------------------------------+-------------------------------------
 * samp           + IN        +  Byte/ Bit                          // binary IN
 * totalc         + IN        +  elcount(buff)                      //
 * ent            + OUT       +  Entropy                            //
 * compRate       + OUT       +  Refer to below                     //
 * chisq          + OUT       +  Chi-square                         //  chip<0.0001, chip>0.9999, else
 * mean           + OUT       +  Mean                               //  (127.5 = random)
 * montepi        + OUT       +  Monte-Carlo-Pi                     //
 * errorPercent   + OUT       +  Deviation                          //
 * scc            + OUT       +  Serial-Correlation                 //   scc >= -99999, else all values 
 * ---------------+-----------+-------------------------------------+------------------------------------*/



void __stdcall ENT_SimpleTest(
...
void __stdcall ENT(                                                                                        // int argc, char *argv[],
...
)
{
    ...
    */
    ...
    /* Not applicable for array inputs
    #ifdef _WIN32

        // Warning! On systems which distinguish text mode and binary I/O (MS-DOS, Macintosh, etc.) the modes in the open
        // statement for "fp" should have forced the input file into binary mode.  But what if we're reading from standard
        // input?  Well, then we need to do a system-specific tweak to make sure it's in binary mode.  While we're at it,
        // let's set the mode to binary regardless of however fopen set it.

        // The following code, conditional on _WIN32, sets binary mode using the method prescribed by Microsoft Visual C 7.0
        // ("Monkey C"); this may require modification if you're using a different compiler or release of Monkey C. If
        // you're porting this code to a different system which distinguishes text and binary files, you'll need to add
    ...
    *chip = pochisq(*chisq, (binary ? 1 : 255));
    /* randomly exceed the Chi-Square value percentage rate = chip * 100 */

    /* Additional parameters output */
    *compRate = (short)((100 * ((binary ? 1 : 8) - *ent) / (binary ? 1.0 : 8.0)));
    *errorPct = 100.0 * (fabs(PI - *montepi) / PI);
    ...
    if (counts) {
        ...
        for (i = 0; i < (binary ? 2 : 256); i++) {
            if (terse) {
                ...
                if (ccount[i] > 0) {
                    // The following expression shows ISO 8859-1 Latin1 characters and blanks out other codes.
                    // The test for ISO space replaces the ISO non-blanking space (0xA0) with a regular
                    // ASCII space, guaranteeing it's rendered properly even when the font doesn't contain
                    // that character, which is the case with many X fonts.
                    printf("%3d   %c   %10ld   %f\n", i, (!isISOprint(i) || isISOspace(i)) ? ' ' : i, 
